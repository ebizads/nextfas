...
function DropZoneComponent(props: { images: ImageJSON[], setImage: React.Dispatch<React.SetStateAction<ImageJSON[]>>, isLoading: boolean, setIsLoading: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  return (<div className="grid grid-cols-2 gap-4 py-2 px-5 col-span-10">
    <div className={`${props.images.length === 0 ? "col-span-2" : "col-span-1"} h-52 rounded-md border bg-white drop-shadow-2xl p-5`}>
      <DropZone setImage={props.setImage} loading={props.isLoading} setIsLoading={props.setIsLoading} />
    </div>
    <div className={`${props.images.length === 0 ? "hidden" : "block"} h-52 duration-200 flex items-center justify-center px-4 py-2 rounded-md border bg-white drop-shadow-2xl`}>
      {props.isLoading === true ? <Loader color="orange" variant="bars" className="self-center" /> : <div className="flex flex-col gap-4 w-[80%] h-48 overflow-y-auto">
        {props.images?.map((img, idx) => <div key={idx} className="flex group justify-between cursor-auto hover:shadow-md hover:border rounded-md">
          ...
          <button onClick={() => handleRemove(idx)} className="w-10 h-full bg-red-500 rounded-br-md rounded-tr-md hidden group-hover:flex items-center justify-center">
